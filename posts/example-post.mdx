---
title: 如何在概述中创建支持SVG的图标符号 第2部分
description: "有些东西一直存在。根据物理学，永远不可能有真正的物理虚无，尽管有时存在类似于虚无，例如真空（可能最小的存在的能量状态）（Phys.org）。创造一个没有量子的空间."
date: 2022 04 22
---

有些东西一直存在。根据物理学，永远不可能有真正的物理虚无，尽管有时存在类似于虚无，例如真空（可能最小的存在的能量状态）（Phys.org）。创造一个没有量子涨落的空间需要大量的能量，如果涨落被冲出，那么这个空间中会有剩余的能量，再加上一个不稳定的环境（1veritasium）。即使在计算机上，删除的数据也不会被丢弃，而是被写下来.
